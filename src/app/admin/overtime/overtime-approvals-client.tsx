...
import { CheckCircle2, Clock3, ListFilter, Paperclip, Timer } from "lucide-react";
...
function getOvertimeHours(request: OvertimeApprovalRow) {
  return Math.max(0, (new Date(request.endAt).getTime() - new Date(request.startAt).getTime()) / (1000 * 60 * 60));
}

function getReviewedDateLabel(request: OvertimeApprovalRow) {
  return request.reviewedAt ? format(new Date(request.reviewedAt), "MMM d, yyyy") : "-";
}

function getSearchText(request: OvertimeApprovalRow) {
  return [
    request.userName,
    request.userEmail,
    request.reason,
...
export function OvertimeApprovalsClient({ initialRequests }: { initialRequests: OvertimeApprovalRow[] }) {
  ...
  async function reviewRequest(requestId: string, nextStatus: ReviewStatus) {
    ...
    const result = await updateOvertimeRequestStatus(requestId, nextStatus);
    if (!result.success) {
      setRequests(previousRequests);
      setError(result.error || "Could not update OT request.");
      return result;
    }
          ...
          <table className="w-full min-w-[1120px] text-left text-sm">
            ...
            <tbody className="divide-y divide-border">
              {visibleRequests.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-12 text-center text-muted-foreground">No OT requests found.</td></tr>
                ...
                <tr key={request.id} className="align-top transition-colors hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">{format(new Date(request.startAt), "MMM d, yyyy h:mm a")} - {format(new Date(request.endAt), "MMM d, yyyy h:mm a")}</td>

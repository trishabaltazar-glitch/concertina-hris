...
function getBreakdownLabel(dayBreakdown: LeaveDaySelection[] | null) {
  ...
  return dayBreakdown
    .map(
      (item) =>
        `${format(parseISO(item.date), "MMM d")} ${item.dayType === "HALF_DAY" ? "half" : "full"}`,
    )
    .join(", ");
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1.5 rounded-md px-2 py-0.5 text-xs font-semibold ring-1",
        styles,
      )}
    >
...
export function LeavesClientPage({
  balances,
  leaveRequests,
}: LeavesClientPageProps) {
  const [statusFilter, setStatusFilter] =
    useState<(typeof STATUS_FILTERS)[number]>("ALL");
  const [dayTypesByDate, setDayTypesByDate] = useState<
    Record<string, LeaveDayType>
  >({});
  ...
  const editingRequest = useMemo(
    () =>
      leaveRequests.find(
        (request) =>
          request.id === editingRequestId && request.status === "PENDING",
      ) || null,
  ...
  function startEditingRequest(request: LeaveRequestItem) {
    ...
    if (request.dayBreakdown?.length) {
      setDayTypesByDate(
        Object.fromEntries(
          request.dayBreakdown.map((item) => [item.date, item.dayType]),
        ),
      );
    } else {
      ...
      setDayTypesByDate(
        Object.fromEntries(
          dates.map((date) => [format(date, "yyyy-MM-dd"), type]),
        ),
      );
    }
  ...
  const requestedDays = selectedBreakdown.reduce(
    (sum, item) => sum + item.days,
    0,
  );
  const requestStartDate = selectedBreakdown[0]?.date || "";
  const requestEndDate =
    selectedBreakdown[selectedBreakdown.length - 1]?.date || "";
  ...
  const pendingCredits = useMemo(() => {
    return leaveRequests.reduce((totals, request) => {
      const days = getRequestDays(
        request.startDate,
        request.endDate,
      ...
      if (request.status === "PENDING") {
        totals.set(
          request.leaveType,
          (totals.get(request.leaveType) ?? 0) + days,
        );
      }
  ...
  const totalPendingCredits = Array.from(pendingCredits.values()).reduce(
    (sum, days) => sum + days,
    0,
  );
          ...
          <form action={handleSubmit} className="space-y-3 p-4">
            ...
            <div className="rounded-lg border border-border p-3">
              ...
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                <div className="rounded-md border border-primary/20 bg-primary/10 px-2 py-1.5 text-primary">
                  Selected full day
                </div>
                <div className="rounded-md border border-amber-300 bg-amber-50 px-2 py-1.5 text-amber-800">
                  Selected half day
                </div>
              </div>
            </div>
                ...
                <div className="max-h-64 divide-y divide-border overflow-y-auto">
                  {selectedBreakdown.map((item) => {
                    return (
                      <div
                        key={item.date}
                        className="grid grid-cols-[1fr_116px_auto] items-center gap-2 px-3 py-2"
                      >
                  ...
                  {selectedBreakdown.length > 0 && (
                    <p
                      className={cn(
                        ...
                        requestedDays > 0 ? "text-primary" : "text-destructive",
                      )}
                    >
                      {requestedDays > 0
                        ? `${requestedDays} PFFD day${requestedDays === 1 ? "" : "s"}`
            ...
            <div>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                ...
                {editingRequest?.attachmentName
                  ? "A new upload is optional because this request already has an attachment."
                  : "Required for every new request."}
              </p>
            </div>
            ...
            <SubmitButton size="sm" disabled={selectedBreakdown.length === 0}>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-1 text-xs leading-5 text-muted-foreground">
                    Available credits are the current balances you can request
                    from each credit type before pending requests are approved.
                  </p>
                </div>
              </div>
            ...
            <section className="rounded-lg border border-border bg-background p-4">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs leading-5 text-muted-foreground">
                    Pending credits are requested days waiting for approval and
                    are not final until reviewed.
                  </p>
                </div>
              </div>
              <div className="mt-4 space-y-2">
                {pendingCredits.size ? (
                  Array.from(pendingCredits.entries()).map(
                    ([leaveType, days]) => (
                      <div
                        key={leaveType}
                          ...
                          <td className="px-4 py-3 text-muted-foreground">
                            ...
                            {request.attachmentName && (
                              <a
                                ...
                                className="mt-1 inline-flex items-center gap-1 text-xs font-medium text-brand-steel hover:text-brand-red"
                              >
                  ...
                  {filteredRequests.map((request) => (
                    ...
                    >
                      ...
                      {request.attachmentName && (
                        <a
                          href={`/leaves/attachments/${request.id}`}
                          className="mt-3 inline-flex items-center gap-1 text-xs font-medium text-brand-steel hover:text-brand-red"
                        >
                          <Paperclip className="size-3" />
                          View attachment
                        </a>
